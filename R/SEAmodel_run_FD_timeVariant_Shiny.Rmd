...
# platy - still a mistery... erepro and Fmort?!
...
# super strange - when I do this run inside project results are differnt because biolevel calculated on B_itime and hence n is different...... 
sim_FD_bmsy@BioLimits[[4]]$SED 
sim@BioLimits[[4]]$SED
...
# scenarios check 
# problems - fleet collapse because more then 5 spp below 20 at some stage, but not in status quo anyway....  
# try df_target_MoreUnderQuota

####### forward price and costs - same as in scenario
...
save.image(file = "/Users/nov017/Dropbox/Mizer-fleet_extension/data/FDData_opn3_May.RData") # was FDData_Nov2.RData or FDData_Nov.RData if using SEAmodel_opn2.RData as input and Tune Q opn2
...
catchComp<-function(sim_FD, df_log_spp){

  # modelled catch by species AND fleet
  trial<-compareTrends(sim_fitted,sim_FD,fleetDynamics = TRUE,type = "yield",yieldObs_timeVariant,ssbObs,rescale=0,areaEco)$trial
...
# plot modelled vs observed
  p <- ggplot(filter(yieldData_sp_fl)) + # fleet =="SET-US"
    geom_bar(aes(x=year, y = yield_tonnes, fill=spCommon), position = "stack", stat = "identity") +
...
catchComp(sim_FD = sim_FD_bmsy, df_log_spp)
