...
load("/Users/nov017/Dropbox/Mizer-fleet_extension/data/FDData_opn3_May_noerepro.RData") 
...
min_w_bb = 1e-4 # OR 1e-7 - see Scenario_final for more comments. min_w_bb # from SEAmodel_run.R - accidentally removed from the list above
...
t_max = 83 # 23 # till 2040 (23) or 2100 (83)
...
######## note - be careful with these values, does this mean that exploitation rate is higher than 1? the values below act as catchability only, but if you add all the Fmort from all the fleets? check that exploitation rate (catch/biomass at t) is lower than 1 
...
save(df_param, areaEco, t_max, scaling_cost_area, sim_FD_bmsy, sim_scenario, target_scenario, file = "/Users/nov017/Dropbox/Mizer-fleet_extension/data/scenarios_original_2100.RData") # if original is until 2040, otherwise it's until 2100
...
# because flethead increase biomass performance without decreasing yield and profit much, in all following scenarios I will use target values for flatheads as per the flathead scenario (no competition). Options are:  
...
# repeat the above for option 2 but then skyp the  ranking and try other options 
# How do I identify spp (e.g. flatheads) that steeply improve either biomass, or yield and profits? 
# just by plotting? 
ggplot(filter(df_plot), aes(x = profit, y = biomassB))+ 
    geom_label(aes(label = scenario))+
...
# run all combination of flathead 0 and other spp 0 
...
load("/Users/nov017/Dropbox/Mizer-fleet_extension/data/scenarios_original_2100.RData") # if original is until 2040, otherwise it's until 2100
...
#-----------------------------------------
# plots of comparison *trial considering scenarios.RData or scenarios_noFlathead.RData - skip* 
#----------------------------------------------

```{r}

# add biomass1, yield1 and  profit1 scenarios - where no interaction for 1 spp (which one depends on the order below) leads to higher biomass, yield, profits. These scenarios are repetitions of the 'species' scenarios, which at this point I am not sure are important 
...
# if using option 2 above - would this change as now yield1 is the one that includes platy + the highest spp in yield? probably no, that would start from yield2 onward...
...
# should we also look at these indicators??? spp below ref and active vessels are an important part as well and decreasing interactions leads to more active fleets .... 
sim_scenario$statusQuo@BioOut 
...
# order fleets 
# c = c("SED","SET-DS","SET-SH","SET-US","SSG") # original 
...
plot_effort = plotFleetEffort(a,b, sim_scenario, sim_FD_bmsy, col_values)
...
# setwd("/Users/nov017/Dropbox/Mizer-fleet_extension/plot/FD/Final")
setwd("/Users/nov017/Dropbox/Mizer-fleet_extension/plot/FD/Final")
# tiff("Fig4.tiff", height=13, width=16, units ='in', res=300)
tiff("FigS9.tiff", height=13, width=16, units ='in', res=300)
plot_matrix / plot_effort / plot_all + plot_layout (nrow = 3, heights = c(2.2,1.4,1.4))
dev.off()

setwd("/Users/nov017/Dropbox/Mizer-fleet_extension/plot/FD/Final")
# pdf("Fig4.pdf", height=13, width=16)
pdf("FigS9.pdf", height=13, width=16)
plot_matrix / plot_effort / plot_all + plot_layout (nrow = 3, heights = c(2.2,1.4,1.4))
dev.off()
...
setwd("/Users/nov017/Dropbox/Mizer-fleet_extension/plot/FD/Final")
# tiff("Fig5.tiff", height=6, width=7, units ='in', res=300) # rescale all indicators 0-1??!  
# lolliIndicator
# dev.off()

setwd("/Users/nov017/Dropbox/Mizer-fleet_extension/plot/FD/Final")
# pdf("Fig5.pdf", height=6, width=7) 
pdf("FigS10.pdf", height=6, width=7) 
lolliIndicator
dev.off()
...
for(i in 1:length(plot_list)){
  ...
  pdf(paste("FigS3_",names(plot_list)[i],".pdf", sep = "" ), height=13, width=13) #, units ='in', res=300)
  print(plot_list[[i]])
  dev.off()
