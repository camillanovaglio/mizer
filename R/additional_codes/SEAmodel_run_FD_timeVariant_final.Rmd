...
# save data to load in NovaglioEtAl_JAEreview and run the model

# clean some data that will need to be used in this script 
# ungroup data first  
datValidationYield<-ungroup(datValidationYield)
datValidationEffort<-ungroup(datValidationEffort)

# observed catch by species and fleet. This is as per code in SEA_FleetParam.Rmd line 1647 to calculate datValidationYieldSpp which then becomes yieldObs_timeVariant in createFmort() in DataFunction.r - thus following same protocol as per all observed data for validation 
...
save(df_param2, df_param, theta, kappa, kappa_ben, kappa_alg, w_pp_cutoff, min_w_bb, w_bb_cutoff, df_selParam_new, df_target_bmsy2, df_price_new, df_cost3, ke_fleet, scaling_price, initial_n, initial_n_bb, initial_n_pp, initial_effort2, scaling_cost_area, areaEco, datValidationYield, datValidationEffort, yieldObs_timeVariant, sim_fitted, df_main_metier, datValidationYieldSppFl, file = "/Users/camillan/R-projects/Mizer-fleet/data/NovaglioetAl_JAEreview.RData")
...
fig3b<-catchComp(sim_FD_bmsy, datValidationYieldSppFl)$p # use datValidationYieldSppFl instead of df_log_spp (see code above - line 394) 
