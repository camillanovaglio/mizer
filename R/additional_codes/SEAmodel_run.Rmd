...
# Error in .Call("_mizer_inner_project_loop", PACKAGE = "mizer", no_sp,  : 
  # "_mizer_inner_project_loop" not available for .Call() for package "mizer"
# but if you do: 'more than one object in chache...'
...
all_mortality<-ungroup(all_mortality) # to fix an error .... 
...
# optimisation criteria *always need to run*  
...
# # upload calibration function
# source("/Users/nov017/R-projects/Mizer-fleet/R/Calibration_function.R")
# 
# # optim function
# optimizer_count=0 # Initialize count of function evaluations
# 
...
# # attempt 1 - OK but much higher kappas, and abundances. thus much higher cacthes for some spp so that observed vs modelled catches is not looking great. better agreeement between observed adn modelled SSB adn worst agreement between observed adn modelled cacthes. Also squalus was left out of the calibration 
# 
# # attempt 2 - calibrate only with catches. reintroduce squalus in the calibration. OK perfect match at eql.  
# save(optim_SEA, file = "optim_SEA_kappa_K_R_opn3.Rdata")

```

# final calibrated community *run*

```{r}

load("/Users/nov017/Dropbox/Mizer-fleet_extension/calibration_local/optim_SEA_kappa_K_R_opn3.Rdata")
...
compareEql_outputs_opn3<-compareEql(areaEco, df_param, sim_calibrated3, constant_effort, df_log_spp, calibration = FALSE)$plot
...
# apply same changes to effort as per constant effort used above
...
# fix squid and others
relative_effort3[,3]<-relative_effort3[,3]*0.5 # squid
df_param[8, "r_max"]<-df_param[8, "r_max"]*0.8 # jackass
df_param[9, "r_max"]<-df_param[9, "r_max"]*0.9 # flathead
df_param[16, "r_max"]<-df_param[16, "r_max"]*0.9 # ling
df_param[17, "r_max"]<-df_param[17, "r_max"]*0.8 # sawshark

# checking mortalities for flathead 
# ft<-all_mortality[which(all_mortality$spe == "platycephalus richardsoni"),]
# view(ft)

params_calibrated4 <- MizerParams(df_param, no_w = no_w, interaction = theta, kappa = kappa, kappa_ben = kappa_ben, kappa_alg = kappa_alg, w_pp_cutoff = w_pp_cutoff, min_w_bb = min_w_bb, w_bb_cutoff = w_bb_cutoff, fleetDynamics = fleetDynamics, selectivity_params = NA, catchability = NA, target = NA)

sim_fitted3 <- project(params_calibrated4, effort = relative_effort3, dt = dt, fleetDynamics = FALSE, multiFleet = FALSE, management = FALSE, price = NA, cost = NA, diet_steps = 2) 
...
# option of line or dots and lines 
...
# problems: 1) arrow squid has high modelled catches; 2) pink ling, blue w and tiger have higher catches in recent years when Fmort for those missing years is calculated given observed cacthes then when Fmore for those years is kept at 0. See options in DataFunction, createFmort() line 182. this only effects the time variant version as the eql and calibration uses 1995-2005 mean values and the FD starts from the time varinat community in 2006. Adjust manually?!  
...
# stop fishing 
no_effort<-rbind(constant_effort3,constant_effort3) # 400 time steps
no_effort[]<-0
matrix_effort_nofishing3<-rbind(constant_effort3, no_effort) # 600 time steps
rownames(matrix_effort_nofishing3)<-seq(1,nrow(matrix_effort_nofishing3))

sim_calibrated_unfished3<- project(params_calibrated3, effort= matrix_effort_nofishing3, dt = dt, fleetDynamics = FALSE, multiFleet = FALSE, management = FALSE, price = NA, cost = NA, diet_steps = 0)
...
pdf("Fig1_Sept2020.pdf", height=6, width=10)
plotYield3
dev.off()
tiff("Fig1_Sept2020.tiff", height=6, width=10, units ='in', res=300)
plotYield3
dev.off()
getwd()
