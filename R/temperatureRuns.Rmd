...
load("/Users/nov017/Dropbox/Mizer-fleet/R/model/FDData_Nov.RData") # from SEAmodel_run_FD_timeVariant_shiny (it is the same sim_calibrated community)
...
rownames(effort_temp)<-1:200
t_max = nrow(sim_calibrated@effort)
...
saveRDS(sim_baserun, file = "SEAustralia_baserun100.RDS")
saveRDS(sim_warming_all, file = "SEAustralia_warming_all100.RDS")
saveRDS(sim_warming_all_eaint, file = "SEAustralia_warming_all_eaint100.RDS")
saveRDS(sim_warming_all_ca, file = "SEAustralia_warming_all_ca100.RDS")
saveRDS(sim_warming_all_eaint_ca, file = "SEAustralia_warming_all_eaint_ca100.RDS")
saveRDS(sim_warming_int, file = "SEAustralia_warming_int100.RDS")
saveRDS(sim_warming_met, file = "SEAustralia_warming_met100.RDS")
saveRDS(sim_warming_mort, file = "SEAustralia_warming_mort100.RDS")

# setwd("/Users/nov017/Dropbox/Warming model comparison/modelOutputs/IrishSea")
# saveRDS(sim_baserun, file = "IrishSea_baserun.RDS")
# saveRDS(sim_warming_all, file = "IrishSea_warming_all.RDS")
# saveRDS(sim_warming_all_eaint, file = "IrishSea_warming_all_eaint.RDS")
# saveRDS(sim_warming_all_ca, file = "SIrishSea_warming_all_ca.RDS")
# saveRDS(sim_warming_all_eaint_ca, file = "IrishSea_warming_all_eaint_ca.RDS")
# saveRDS(sim_warming_int, file = "IrishSea_warming_int.RDS")
# saveRDS(sim_warming_met, file = "IrishSea_warming_met.RDS")
# saveRDS(sim_warming_mort, file = "IrishSea_warming_mort.RDS")

```

# to do 
not much is changing - which plot should you use to check temp effects? 
plot growth curves to see if they work - if the species argument is specified you get a colored curve which is the one modelled and a black curve which is the vonbertalanffy growth given k_vb, a and b and age (not sure how age is calcualted, but seems easy). the modelled growth is caclaulted given the available energy to grow (instead of reproduce), which depends on food consumption and maturation level. K_vb in the model is only used to calcualte h (maximum consumption rate) if h is not a column of the df_param and to compare modelled growth with von bertalanffy growth in this plot - von bertalanffy growth may have a strance shape because k_vb is often small and growth is slow - but no problem as k_vb is actually not used in the model (to recheck but quite sure).   
